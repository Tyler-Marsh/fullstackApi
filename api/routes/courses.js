...
// update query to include information from the Users table
router.get('/courses/:id', asyncHandler(async (req, res) => {
 // course = await Course.findByPk(req.params.id);
 const id = req.params.id;
 course = await db.sequelize.query(`SELECT Courses.*, Users.firstName, Users.lastName, Users.emailAddress from Courses, Users WHERE Courses.id = ${id} AND Courses.userId = Users.id `);
 result = await course[0];
  res.json({result})
}));
